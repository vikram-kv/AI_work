...
class MyBot : public OthelloPlayer
{
    ...
    private:
        ...
        double evalGridBegin(OthelloBoard& board);
        double evalGridEnd(OthelloBoard& board);
        double heuristic(OthelloBoard& oboard);
};

class sortMoves{
    public:
        sortMoves(OthelloBoard, Turn);
        list<Move> sortList(list<Move> moves);
    private:
        OthelloBoard board;
        Turn myColor;
        int heuristic(Move);
        static bool comp(pair<Move,int>,pair<Move,int>);
};
sortMoves::sortMoves(OthelloBoard b, Turn myColor){
    this->board = b;
    this->myColor = myColor;
}
list<Move> sortMoves::sortList(list<Move> moves){
    vector<pair<Move,int>> mv;
    for(auto it = moves.begin(); it!=moves.end();++it){
        mv.push_back(make_pair(*it,heuristic(*it)));
    }
    sort(mv.begin(), mv.end(), comp);
    list<Move> reorderedMoves;
    for(int i=0;i<(int)mv.size();i++){
        reorderedMoves.push_back(mv[i].first);
    }
    return reorderedMoves;
}
bool sortMoves::comp(pair<Move,int> a, pair<Move, int> b){
    return a.second <= b.second;
}
int sortMoves::heuristic(Move a){
    OthelloBoard bcopy= board;
    bcopy.makeMove(myColor, a);
    return bcopy.getValidMoves(other(myColor)).size();
}
...
double MyBot::evalGridBegin(OthelloBoard& oboard) {
    ...
    if (aliceFrontierCoins > bobFrontierCoins) coindiffscore = -100.0 * aliceFrontierCoins / (aliceFrontierCoins + bobFrontierCoins);
    else if (bobFrontierCoins > aliceFrontierCoins) coindiffscore = 100.0 * bobFrontierCoins / (aliceFrontierCoins + bobFrontierCoins);
    ...
    if (aliceMoves > bobMoves) coindiffscore = 100.0 * aliceMoves / (aliceMoves + bobMoves);
    else if (bobMoves > aliceMoves) coindiffscore = -100.0 * bobMoves / (aliceMoves + bobMoves);
    ...
    double finalscore = 85 * cornerscore + 38 * nearcornerscore + 10 * mobilityscore + 8 * coinfrontierdiffscore + 
                        coinweightscore + coindiffscore;
    return finalscore;
}

double MyBot::evalGridEnd(OthelloBoard& oboard) {
    vector<vector<char>> lboard = getLabelGrid(oboard, myColor);
    int aliceCoins = 0, bobCoins = 0;
    double coindiffscore = 0.0;
    for(int i=0;i<8;++i) {
        for(int j=0;j<8;++j) {
            char cell = lboard[i][j];
    ...
    coindiffscore = 1000*(aliceCoins - bobCoins);
    return coindiffscore;
}

double MyBot::heuristic(OthelloBoard& oboard) {
    int coins = oboard.getBlackCount() + oboard.getRedCount();
    if (coins >= 56) {
        return evalGridEnd(oboard);
    } else {
        return evalGridBegin(oboard);
    }
}
...
double MyBot::alphaBeta(OthelloBoard &board, int ptype, double alpha, double beta, int depth) {
    ...
    if(depth==MAXDEPTH){
        sortMoves s(board,pColor);
        moves = s.sortList(moves);
    }
