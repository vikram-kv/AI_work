...
class TSP {
    ...
    set<set<set<int>>> lkhSolutions;
    set<set<int>> curSolution;
    vector<int> curTour;
    real curCost;
    int runcounter;
...
public:
    ...
    set<int> adjVertices(set<set<int>>& tour, int v) {
        set<int> answer = {};
        for(auto e : tour) {
            if (e.find(v) != e.end()) {
                answer.insert(e.begin(), e.end());
            }
        }
        answer.erase(v);
    ...
    vector<pair<int,pair<real, real>>> bestNeighbours(set<set<int>>& tour, int lv, real gain, set<set<int>>& addedEdges, set<set<int>>& removedEdges) {

        map<int, pair<real, real>> answer = {};
        int n = citycount;
        for(int i=0;i<n;++i) {
            if (i == lv) continue;
            set<int> candEdge({i, lv});
    ...
    bool lkh_boost() {
        ...
        set<set<int>> tourEdges = curSolution;

        for(auto u : tour) {
            auto t =  adjVertices(tour, u);
            for(auto v : t) {
                set<set<int>> removedEdges;
                removedEdges.insert({u,v});
                

                for(auto r : removedEdges) {
                    assert(r.size() == 2);
                }


                set<set<int>> addedEdges;
                ...
                for(auto& n : bestn) {
                    ...
                    addedEdges.insert({v,w});
                    for(auto r : addedEdges) {
                        assert(r.size() == 2);
                    }
    ...
    pair<bool, set<set<int>>> makeTour(set<set<int>>& tour, set<set<int>>& addedEdges, set<set<int>>& removedEdges) {
        ...
        return {ok, set<set<int>>(newtour.begin(), newtour.end())};
    }

    bool removeEdge(set<set<int>>& tourEdges, int fv, int lv, real gain, set<set<int>>& addedEdges, set<set<int>>& removedEdges) {
        ...
        for(auto n : rmCands) {
            ...
            set<set<int>> rmedgesc = removedEdges;
            rmedgesc.insert(rmEdge);
                    for(auto r : rmedgesc) {
                        assert(r.size() == 2);
                    }
            set<set<int>> aadedgesc = addedEdges;
            aadedgesc.insert({n,fv});
                    for(auto r : aadedgesc) {
                        assert(r.size() == 2);
                    }
    ...
    bool addEdge(set<set<int>>& tourEdges, int fv, int lv, real gain, set<set<int>> addedEdges, set<set<int>> removedEdges) {
        ...
        for(auto& n : bestGains) {
            real gainn = n.second.second;
            set<int> add({n.first, lv});

            set<set<int>> nadded(addedEdges);
            nadded.insert(add);
                    for(auto r : nadded) {
                        assert(r.size() == 2);
                    }
